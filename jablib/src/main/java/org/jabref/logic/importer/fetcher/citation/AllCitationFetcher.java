...
public class AllCitationFetcher implements CitationFetcher {
    ...
    public AllCitationFetcher(ImporterPreferences importerPreferences,
    ...
    public List<BibEntry> getReferences(BibEntry entry) throws FetcherException {
        return deduplicate(fetch(fetcher -> fetcher.getReferences(entry)));
    }

    @Override
    public List<BibEntry> getCitations(BibEntry entry) throws FetcherException {
        return deduplicate(fetch(fetcher -> fetcher.getCitations(entry)));
    }
    ...
    private List<BibEntry> fetch(FetchOperation operation) {
        ...
        for (CitationFetcher fetcher : fetchers) {
            try {
                ...
                LOGGER.warn("Fetching from {} failed — continuing", fetcher.getName(), e);
            }
        }
        return result;
    }
    ...
    private List<BibEntry> deduplicate(List<BibEntry> entries) {
        ...
        BibDatabaseMode mode = BibDatabaseModeDetection.inferMode(temp);

        List<BibEntry> unique = new ArrayList<>();
        for (BibEntry entry : entries) {
            if (duplicateCheck.containsDuplicate(temp, entry, mode).isEmpty()) {
